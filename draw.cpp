...
void drawImage(SDL_Renderer *renderer, const char *path, int x, int y, int w, int h)
{
...
void writeText(SDL_Renderer *renderer, const char *fontPath, const char *text, int x, int y, int size, int r, int g, int b, int a)
{
    if (!text || text[0] == '\0')
    {
...
void drawTopPlayers(SDL_Renderer *renderer, game *Game)
{
    ...
    if (!loaded || Game->helpers.updatedTopPlayers)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            sprintf(text, "%s %d %d-%d-%d", players[i].name, players[i].score, players[i].date.day, players[i].date.month , players[i].date.year);
            ...
            sprintf(text, "%d-%d-%d", players[i].date.day, players[i].date.month , players[i].date.year);
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        if (i < 3)
        {
            const char *iconPath = i == 0 ? FIRST_ICON_PATH : i == 1 ? SECOND_ICON_PATH
                                                               : THIRD_ICON_PATH;
...
int mesureTextWidth(const char *fontPath, const char *text, int size)
{
    TTF_Font *font = TTF_OpenFont(fontPath, size);
    if (!font)
    {
